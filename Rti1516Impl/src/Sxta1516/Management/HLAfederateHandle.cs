namespace Sxta.Rti1516.Management
{
    ...
    public class HLAhandleList : List<HLAfederateHandle>
    {
        ...
        // no propaga los cambios. SOLUCIÓN: En el MetaFederateAmbassador se intercepta
        // la asignación al federado de su HLAfederateHandle. En ese momento se actualiza la lista local
        // del objeto federado (HLAfederatesinFederation)

        public override string ToString()
        {
            String returned = "HLAhandleList[";

            for (int i = 0; i < Count; i++)
            {
                HLAfederateHandle federate = this[i];
                if (i + 1 == Count)
                    returned = returned + federate.ToString();
                else
                    returned = returned + federate.ToString() + ", ";
            }
